...
export default function PlaygroundCredit() {
  ...
  const labelStyles = "mb-1.5 block text-xs font-mono tracking-wider text-zinc-400 uppercase";
    ...
    <div className="w-full max-w-4xl mx-auto mt-8">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-zinc-700 bg-gradient-to-tr from-zinc-950 via-zinc-900 to-zinc-800 p-8 shadow-2xl sm:p-10">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <svg className="h-12 w-12 text-zinc-500 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <input type="number" name="Duration" value={formData.Duration} onChange={handleChange} min="1" max="72" placeholder="e.g. 24" className={inputStyles} />
          </div>

          <div>
            <label className={labelStyles}>Purpose</label>
            <select name="Purpose" value={formData.Purpose} onChange={handleChange} className={inputStyles}>
          ...
          <div>
            ...
            <input type="number" name="Age" value={formData.Age} onChange={handleChange} min="18" max="100" placeholder="e.g. 35" className={inputStyles} />
          </div>
          ...
          <div>
            ...
            <select name="Housing" value={formData.Housing} onChange={handleChange} className={inputStyles}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <input type="number" name="Job" value={formData.Job} onChange={handleChange} min="0" max="3" placeholder="0-3" className={inputStyles} />
          </div>

          <div>
            <label className={labelStyles}>Saving Accounts</label>
            <select name="Saving accounts" value={formData["Saving accounts"]} onChange={handleChange} className={inputStyles}>
          ...
          <div>
            ...
            <select name="Checking account" value={formData["Checking account"]} onChange={handleChange} className={inputStyles}>
              <option value="NA">NA</option>
              <option value="little">Little</option>
              <option value="moderate">Moderate</option>
              <option value="rich">Rich</option>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-6 border-t border-zinc-700/50 pt-8 md:flex-row">
          ...
          <button
            ...
            disabled={loading}
            className={`w-full md:w-auto rounded-lg px-8 py-3.5 font-bold transition-all ${loading
      ...
      {result && (
        <div className="mx-auto mt-6 w-full max-w-4xl overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl">
          <div className="border-b border-zinc-800 bg-zinc-900 px-4 py-2">
            <span className="font-mono text-xs text-zinc-500">Output Terminal</span>
          </div>
